  ...
  head: () => ({
    meta: [
      { title: "CeeVeeForYou — Professional CVs Built For Success" },
      {
        name: "description",
        content:
          "AI-powered, ATS-friendly CV builder. Elegant one-page resumes in minutes. Mobile-first and installable.",
      },
    ],
  }),
});
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <Link to="/signup" className="rounded-sm bg-foreground px-3 py-1.5 text-background">
              Start free
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-6 pb-24 pt-16 lg:grid-cols-[1.05fr_0.95fr] lg:pt-24">
        <div>
          ...
          <p className="mt-6 max-w-xl text-base text-muted-foreground sm:text-lg">
            CeeVeeForYou writes, formats, and compresses your résumé into a pixel-perfect, ATS-safe
            single page — set in Arial, ready for any applicant tracking system.
          </p>
          <div className="mt-9 flex flex-wrap items-center gap-4">
            <Link
              to="/signup"
              className="rounded-sm bg-foreground px-6 py-3 text-sm font-medium text-background"
            >
              Create my CV →
            </Link>
          ...
          <dl className="mt-12 grid max-w-md grid-cols-3 gap-6 border-t border-border pt-6 text-sm">
            <div>
              ...
              <dd className="mt-1 font-medium">A4 · Arial</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">Pages</dt>
              <dd className="mt-1 font-medium">One, always</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">ATS</dt>
              <dd className="mt-1 font-medium">100% safe</dd>
            </div>
          </dl>
        </div>
      ...
      <section id="features" className="border-y border-border bg-card">
        <div className="mx-auto grid max-w-6xl gap-px bg-border px-0 sm:grid-cols-2 lg:grid-cols-4">
          {[
            [
              "One-page optimiser",
              "Auto-tunes spacing, font weight, and bullet density to keep your CV on a single page.",
            ],
            [
              "ATS-safe by default",
              "No tables, no columns, no icons in the export. Just clean Arial that every parser reads.",
            ],
            [
              "AI writing assistant",
              "Generate summaries and achievement-led bullets that sound like you — only sharper.",
            ],
            [
              "Offline PWA",
              "Install on iOS or Android. Edit and review your last CV with no signal.",
            ],
          ].map(([t, d]) => (
        ...
        <ol className="mt-12 grid gap-10 sm:grid-cols-3">
          {[
            [
              "01",
              "Enter your details",
              "Type or paste. The editor mirrors the final document live.",
            ],
            [
              "02",
              "Let AI sharpen it",
              "Generate a summary and rewrite bullets with measurable outcomes.",
            ],
            [
              "03",
              "Download as PDF",
              "Exports Arial, A4, with a camelCase filename ready to send.",
            ],
          ].map(([n, t, d]) => (
            <li key={n}>
          ...
          <div className="mt-10 divide-y divide-border">
            {[
              [
                ...
                "Yes. The export uses Arial, semantic structure, no tables or images, and a single column — the four things every modern parser depends on.",
              ],
              [
                "Will my CV fit on one page?",
                "The one-page optimiser tightens spacing and typography until it does. You can also toggle this off.",
              ],
              [
                "Do I need to add dates?",
                "No — dates are optional everywhere. Toggle them per-CV from the settings.",
              ],
              [
                "Does it work offline?",
                "Once installed as a PWA, your dashboard and last-opened CVs are available without a connection.",
              ],
            ].map(([q, a]) => (
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-3 px-6 py-10 text-sm text-muted-foreground sm:flex-row sm:items-center">
          ...
          <div>
            Built by <span className="text-foreground">Architeq Web Agency</span>
          </div>
        </div>
      </footer>
    </div>
