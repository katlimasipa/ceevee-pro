...
function CVBuilder() {
  ...
  const update = <K extends keyof CVData>(k: K, v: CVData[K]) =>
    setCv((c) => c && { ...c, [k]: v });
  ...
  const downloadPdf = async () => {
    ...
    try {
      if (user)
        await supabase
          .from("cv_events")
          .insert({ user_id: user.id, cv_id: id, event_type: "download" });
    } catch {}
    window.print();
  };
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {saving === "saving" ? "Saving…" : "All changes saved"} · One-page mode{" "}
            {cv.settings.onePage ? "on" : "off"}
          </p>
        </div>
        <div className="flex gap-2">
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={cv.settings.showDates}
              onChange={(e) => update("settings", { ...cv.settings, showDates: e.target.checked })}
            />
            Show dates
          </label>
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={cv.settings.onePage}
              onChange={(e) => update("settings", { ...cv.settings, onePage: e.target.checked })}
            />
            One-page
          </label>
          <button
            onClick={downloadPdf}
            className="rounded-sm bg-foreground px-4 py-2 text-sm font-medium text-background"
          >
            Download PDF
          </button>
        </div>
      ...
      <div className="mb-4 flex gap-2 lg:hidden">
        {(["edit", "preview"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 rounded-sm border px-3 py-2 text-sm ${tab === t ? "border-foreground bg-foreground text-background" : "border-border"}`}
          >
            {t === "edit" ? "Edit" : "Preview"}
          </button>
        ))}
...
function Editor({
  ...
  const doGenSummary = async () => {
    setGenSum(true);
    try {
      const expSnips = cv.experience
        .map((e) => `${e.position} @ ${e.company}: ${e.bullets.slice(0, 2).join(" ")}`)
        .join(" | ");
      const skSnips = cv.skills.flatMap((g) => g.items).join(", ");
      const { summary } = await aiSummary({
        data: { titles: p.titles, experience: expSnips, skills: skSnips },
      });
  ...
  return (
    <div className="space-y-10">
      ...
      <Section title="Personal information">
        ...
        <In
          label="Professional titles (separate with | )"
          value={(p.titles ?? []).join(" | ")}
          onChange={(v) =>
            setP({
              titles: v
      ...
      >
        <textarea
          ...
          rows={5}
          className="w-full resize-y border border-border bg-paper p-3 text-sm outline-none focus:border-foreground"
      ...
      <Section
        ...
        action={
          <button
            onClick={() => update("education", [...cv.education, { institution: "", degree: "" }])}
            ...
            >
              <div className="grid gap-3 sm:grid-cols-2">
                <In
                  ...
                  value={e.institution}
                  onChange={(v) =>
                    updateAt<EducationItem>(cv.education, i, { institution: v }, (arr) =>
                ...
                <In
                  ...
                  value={e.location ?? ""}
                  onChange={(v) =>
                    updateAt<EducationItem>(cv.education, i, { location: v }, (arr) =>
                      update("education", arr),
                    )
                  }
                />
                <In
                  ...
                  value={e.degree}
                  onChange={(v) =>
                    updateAt<EducationItem>(cv.education, i, { degree: v }, (arr) =>
                      update("education", arr),
                    )
                  }
                />
                <In
                  ...
                  value={e.dates ?? ""}
                  onChange={(v) =>
                    updateAt<EducationItem>(cv.education, i, { dates: v }, (arr) =>
                      update("education", arr),
                    )
                  }
                />
      ...
      <Section
        ...
        action={
          <button
            onClick={() =>
              update("experience", [...cv.experience, { company: "", position: "", bullets: [""] }])
            }
            className="text-xs underline-offset-4 hover:underline"
          >
            + Add role
        ...
        <div className="space-y-5">
          {cv.experience.map((x, i) => (
            <ExperienceCard
              ...
              item={x}
              onChange={(patch) =>
                updateAt<ExperienceItem>(cv.experience, i, patch, (arr) =>
      ...
      <Section
        ...
        action={
          <button
            onClick={() => update("skills", [...cv.skills, { group: "", items: [] }])}
            className="text-xs underline-offset-4 hover:underline"
          >
            + Add group
          </button>
          ...
          {cv.skills.map((g, i) => (
            ...
            >
              <In
                ...
                value={g.group}
                onChange={(v) =>
                  updateAt(cv.skills, i, { group: v }, (arr) => update("skills", arr))
                }
              />
              <In
...
function ExperienceCard({
  ...
  const gen = async () => {
    ...
    setBusy(true);
    try {
      const { bullets } = await aiBullets({
        data: { position: item.position, company: item.company, context: item.bullets.join(" ") },
      });
      ...
      <div className="mt-3">
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <div className="flex gap-3 text-xs">
            ...
            >
              {busy ? "Generating…" : "✦ AI bullets"}
            </button>
            <button
              onClick={() => onChange({ bullets: [...item.bullets, ""] })}
          ...
          {item.bullets.map((b, j) => (
            <div key={j} className="flex gap-2">
              <textarea
                value={b}
                onChange={(e) =>
                  onChange({ bullets: item.bullets.map((x, k) => (k === j ? e.target.value : x)) })
                }
                rows={1}
                className="w-full resize-none border border-border bg-paper p-2 text-sm outline-none focus:border-foreground"
              />
              <button
                onClick={() => onChange({ bullets: item.bullets.filter((_, k) => k !== j) })}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full border border-border bg-paper p-2 text-sm outline-none focus:border-foreground"
      />
    </label>
  );
}
