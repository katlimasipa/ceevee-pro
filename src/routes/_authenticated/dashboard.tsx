  ...
  const { data: cvs = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("cvs")
        .select("id, title, updated_at, personal_info")
    ...
    mutationFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          user_id: user!.id,
          title: base.title,
          personal_info: base.personal_info as any,
          summary: base.summary,
          education: base.education as any,
          experience: base.experience as any,
          skills: base.skills as any,
      ...
      await supabase
        .from("cv_events")
        .insert({ user_id: user!.id, cv_id: data.id, event_type: "create" });
      return data.id;
    },
    onSuccess: (id) => {
      qc.invalidateQueries({ queryKey: ["cvs"] });
      nav({ to: "/cv/$id", params: { id } });
    },
  ...
  const dupMut = useMutation({
    mutationFn: async (id: string) => {
      ...
      const { data: created, error: ce } = await supabase
        .from("cvs")
        .insert({ ...rest, title: `${orig.title} (copy)` })
        .select("id")
        .single();
      if (ce) throw ce;
      return created.id;
        ...
        {isLoading ? (
          ...
          <div className="rounded-sm border border-dashed border-border p-16 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Create your first résumé to get started.
            </p>
            <button
              onClick={() => createMut.mutate()}
              className="mt-6 rounded-sm bg-foreground px-5 py-2.5 text-sm text-background"
            >
              Create my first CV
            </button>
          </div>
              ...
              <li key={cv.id} className="flex flex-wrap items-center justify-between gap-3 py-5">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {cv.personal_info?.fullName || "Untitled"} · Updated{" "}
                ...
                <div className="flex gap-2 text-sm">
                  <Link
                    to="/cv/$id"
                    params={{ id: cv.id }}
                    className="rounded-sm border border-border px-3 py-1.5 hover:bg-accent"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => dupMut.mutate(cv.id)}
                    className="rounded-sm border border-border px-3 py-1.5 hover:bg-accent"
                  >
                    Duplicate
                  </button>
                  <button
                    onClick={() => confirm("Delete this CV?") && delMut.mutate(cv.id)}
                    className="rounded-sm border border-border px-3 py-1.5 text-destructive hover:bg-accent"
                  >
                    Delete
                  </button>
                </div>
